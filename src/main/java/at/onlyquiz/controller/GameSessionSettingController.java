...
public class GameSessionSettingController extends BaseController {
    @FXML
    public Button defaultModeButton, endlessModeButton, backButton;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        for (var k : keySet) {
            questionnaireSelection_obstList.add(new QuestionnaireSelection(k, false));
        }
        ...
        if (current_user.getHashedPassword() != null) {
            if (current_user.get_selectedQuestionnaires().isEmpty() && current_user.get_selectedProperties().isEmpty())
                return;
    ...
    private List<String> get_selectedQuestionnairs() {
        ...
        for (var item : questionnaireSelection_obstList) {
            if (item.isSelected()) selectedQuestionnairs.add(item.get_questionnaireName());
        }
